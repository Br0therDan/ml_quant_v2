...
with res_col:
    with st.container(border=True, height="stretch"):
        if not sel_strat or not sel_asof:
            ...
            if approved_only:
                df_curr = df_curr[df_curr["approved"] == True]
        ...
        with tab1:
            ...
            top_syms = df_curr[df_curr["approved"] == True].head(5)["symbol"].tolist()
            if top_syms:
                st.write(
                    "**Top Approved:** " + " ".join([f"`{s}`" for s in top_syms])
                )
            st.dataframe(
                format_df_for_display(df_curr), width="stretch", hide_index=True
            )
        ...
        with tab2:
            with st.container(border=True):
                ...
                if df_comp.empty:
                    ...
                    new_symbols = df_comp[df_comp["weight_prev"] == 0][
                        "symbol"
                    ].tolist()
                    rem_symbols = df_comp[df_comp["weight_curr"] == 0][
                        "symbol"
                    ].tolist()
